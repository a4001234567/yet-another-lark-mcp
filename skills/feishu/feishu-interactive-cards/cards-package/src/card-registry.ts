...
export type CardEntry = {
  ...
  submit_label?: string;
  expiry_at:     number;
  // Blocking mode — resolved by callback, rejected by expiry
...
type ConfirmBase = Pick<CardEntry, 'title' | 'body' | 'confirm_label' | 'cancel_label'>;

export function buildConfirmActiveCard(e: ConfirmBase): object {
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { template: 'red', title: { tag: 'plain_text', content: e.title } },
    body: {
      elements: [
...
/** WS inline response for card.action.trigger — returned directly to Lark within 3 s. */
export function buildCallbackResponse(entry: CardEntry, action: string): object {
  return {
    toast: {
      type:    action === 'confirm' ? 'success' : 'info',
      content: action === 'confirm' ? '已确认' : '已取消',
    },
    card: {
      type: 'raw',
      data: buildConfirmRespondedCard(entry, action),
    },
  };
}
